...
class Staff extends AdminController
{
    ...
    public function member($id = '')
    {
        ...
        if ($id == '') {
            ...
            $data['staff_activity_log']  = $this->staff_model->get_staff_activity_log($id);
            // get tasks


            /** 
             * End staff activety log updte 3.0.9. 
             */
            if (!$member) {
        ...
        $returnVal = $this->getAllTasks($id);
        $data['members']  =  $returnVal['members'];
        $data['overview'] =   $returnVal['overview'];
        $data['years']  = $returnVal['years'];
        $data['staff_activity_log']  = $this->staff_model->get_staff_activity_log($id);

        $this->load->view('admin/staff/member', $data);
    }

    public function getAllTasks($staffid = '')
    {
        $overview = [];
        ...
        $showAllMonth = ($this->input->post('month') ? false : true);
