...
class Misc extends AdminController
{
    ...
    public function get_relation_data()
    {
        if ($this->input->post()) {
            ...
            //$relOptions = array(array("id" => "19" , "name" => "#19 -  P1020 - 2 PANELS - DUBAI ELECTRICITY & WATER AUTHORITY" , "link" => "http:\/\/localhost\/Prizm_crm\/admin\/projects\/view\/19" , "addedfrom" => 0 , "subtext" => "" ,"type" => "project") );
            $relOptions =   array()  ;
            $oppo = $this->list_active_oppurtunities();
foreach ($oppo as $opp){
$relOption = array("id" => "".$opp["opportunity_id"] , "name" => "#".$opp["opportunity_id"]." - ".$opp["opportunity_code"]." - ".$opp["opportunity_name"]." - ".$opp["partner_code"] , "link" => admin_url("/opportunities/opportunity_details/".$opp["opportunity_id"]) , "addedfrom" => 0 , "subtext" => "" ,"type" => "opportunity");
array_push($relOptions , $relOption);
            }
            }//end else
...
public function list_active_oppurtunities(){

    $data = $this->misc_model->get_advanceleads_statuses2();
    return $data;
}
