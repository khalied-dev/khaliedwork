...
class Tasks extends AdminController
{
    ...
    /**   
     * Start task activety log updte 3.0.9.   
     *   
     * @author  @Ahmed Hmed   
     * @version 1.0   
     * @since   2023-8-2   
    ...
    public function update_task_description($id)
    {
        if (has_permission('tasks', '', 'edit')) {
            ...
            $this->AddTaskLog("Task description has been updated", $id);
            // end active log 
    ...
    /**
     * End task activety log updte 3.0.9.
     */
    ...
    public function task($id = '')
    {
        ...
        if ($this->input->post()) {
            ...
            if ($id == '') {
                ...
                $this->AddTaskLog("Task has been added", $id);
            } else {
                if (!has_permission('tasks', '', 'edit')) {
                    header('HTTP/1.0 400 Bad error');
                    echo json_encode([
                ...
                $this->AddTaskLog("Task has been updated", $id);
            }
            die;
        }
    ...
    public function get_task_data($taskid, $return = false)
    {
        ...
        $this->load->model('Task_Activity_Log_model');
        $tasklogs = $this->Task_Activity_Log_model->get_Alltask_log_detail($taskid);
    ...
    public function add_reminder($task_id)
    {
        ...
        $this->AddTaskLog("Reminder has been added", $task_id);
    }
    ...
    public function edit_reminder($id)
    {
        ...
        $this->AddTaskLog("Reminder has been modified", $reminder->rel_id);
    }
    ...
    public function delete_reminder($rel_id, $id)
    {
        ...
        $this->AddTaskLog("Reminder has been deleetd", $rel_id);
    ...
    public function add_checklist_item()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                ...
                $this->AddTaskLog("New checklist item has been added.", $this->input->post('taskid'));
            }
        }
    }
    ...
    public function update_checklist_order()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                ...
                $this->AddTaskLog("Checklist item has been updated.", $taskid);
            }
        }
    }
    ...
    public function delete_checklist_item($id)
    {
        ...
        if (has_permission('tasks', '', 'delete') || $list->addedfrom == get_staff_user_id()) {
            if ($this->input->is_ajax_request()) {
                ...
                $this->AddTaskLog("Checklist item has been deleted.", $taskid);
            }
        }
    }
    ...
    public function update_checklist_item()
    {
        if ($this->input->is_ajax_request()) {
            if ($this->input->post()) {
                ...
                $this->AddTaskLog("Checklist item order has been updated.", $taskid);
            }
        }
    }
    ...
    public function make_public($task_id)
    {
        ...
        $this->AddTaskLog("Task marked as public", $task_id);
    }
    ...
    public function add_task_comment()
    {
        ...
        $this->AddTaskLog("New comment added", $this->input->post('taskid'));
    }
    ...
    public function add_task_followers()
    {
        ...
        if (
            ...
            $this->AddTaskLog("New task follower has been added", $this->input->post('taskid'));
        }
    }
    ...
    public function add_task_assignees()
    {
        ...
        if (
            ...
            $this->AddTaskLog("New task asignee has been added", $this->input->post('taskid'));
        }
    }
    ...
    public function edit_comment()
    {
        if ($this->input->post()) {
            ...
            $this->AddTaskLog("Comment modified", $data['task_id']);
        }
    }
    ...
    public function remove_comment($id)
    {
        ...
        $this->AddTaskLog("Comment deleted.", $taskid);
    }
    ...
    public function remove_assignee($id, $taskid)
    {
        ...
        if (
            ...
            $this->AddTaskLog("Task asignee has been removed", $taskid);
        }
    }
    ...
    public function remove_follower($id, $taskid)
    {
        ...
        if (
            ...
            $this->AddTaskLog("Task follower has been removed", $taskid);
        }
    }
    ...
    public function unmark_complete($id)
    {
        if (
            ...
            $this->AddTaskLog("Task marked as uncomplete", $id);
        }
    }
    ...
    public function mark_as($status, $id)
    {
        if (
            ...
            if ($success) {
                // TODO: Add a hook here to allow other modules to add their own messages
                $this->AddTaskLog("Status update. Task marked as ", $id);
    ...
    public function change_priority($priority_id, $id)
    {
        ...
        $this->AddTaskLog("Task periority has been changed  ", $id); //TODO: add periority
    }
    ...
    public function task_single_inline_update($task_id)
    {
        if (has_permission('tasks', '', 'edit')) {
            ...
            foreach ($post_data as $key => $val) {
                ...
                $this->AddTaskLog("Task date updated", $task_id);
            }
        }
    }
    ...
    public function remove_task_attachment($id)
    {
        if ($this->input->is_ajax_request()) {
            ...
            $this->AddTaskLog("Task attachment removed", $id);
        }
    }
    ...
    public function upload_file()
    {
        if ($this->input->post()) {
            ...
            $this->AddTaskLog("Task attachment uploaded", $taskid);
        }
    }
    ...
    public function timer_tracking()
    {
        ...
        if ($this->input->post('timer_id'))
            $this->AddTaskLog("Task timer stopped", $task_id);
        else
            $this->AddTaskLog("Task timer started", $task_id);
    }
    ...
    public function delete_user_unfinished_timesheet($id)
    {
        ...
        $this->AddTaskLog("One unfinished task timeshet deleted", $id);
    }
    ...
    public function delete_timesheet($id)
    {
        if (staff_can('delete_timesheet', 'tasks') || staff_can('delete_own_timesheet', 'tasks') && total_rows(db_prefix() . 'taskstimers', ['staff_id' => get_staff_user_id(), 'id' => $id]) > 0) {
            ...
            $taskid = $this->tasks_model->getTaskID($id, "taskstimers", "id", "task_id");
            $this->AddTaskLog("Timeshet deleted", $taskid);
    ...
    public function update_timesheet()
    {
        if ($this->input->is_ajax_request()) {
            if (staff_can('edit_timesheet', 'tasks') || (staff_can('edit_own_timesheet', 'tasks') && total_rows(db_prefix() . 'taskstimers', ['staff_id' => get_staff_user_id(), 'id' => $this->input->post('timer_id')]) > 0)) {
                ...
                $this->AddTaskLog("Task timeshet updated", $this->input->post('timesheet_task_id'));
    ...
    public function log_time()
    {
        ...
        $this->AddTaskLog("Task timesheet added", $this->input->post('timesheet_task_id'));
    ...
    public function update_tags()
    {
        if (has_permission('tasks', '', 'create') || has_permission('tasks', '', 'edit')) {
            ...
            $this->AddTaskLog("Task tag has been updated", $id);
        }
    }
    ...
    public function bulk_action()
    {
        ...
        if ($this->input->post()) {
            ...
            if (is_array($ids)) {
                ...
                $this->AddTaskLog("Bulk action perfomed on task", $id);
            }
            if ($this->input->post('mass_delete')) {
    ...
    public function AddTaskLog($logdetail, $taskid)
    {
        $data['logdetail'] = $logdetail;
        $data['fktaskid'] = $taskid;
        $data['fkstaffid'] = get_staff_user_id();
        $data['adddate'] = date('Y-m-d H:i:s');
        $this->load->model('Task_Activity_Log_model');
        $added = $this->Task_Activity_Log_model->add($data);
    }
    ...
    /**   
     * Start task activety log updte 3.0.9.
     * change task status to need approved   
     *   
     * @author  @Ahmed Hmed   
     * @version 1.0   
     * @since   2023-8-2   
    ...
    function send_to_approval()
    {
        if ($this->input->is_ajax_request()) {
            ...
            $task = $this->tasks_model->get($taskid);
            $task->status = 3;
            $task->datefinished = date('Y-m-d H:i:s');
            $this->tasks_model->update($taskid, $task);
            $this->AddTaskLog("Task send to approval", $taskid);

            $notifiedUsers = [];
            // ارسال اشعار الى الشخص الذى قام بانشاء المهمه
            $notification_data = [
                'description' => 'not_task_send_to_approval',
                'touserid'    => $task->addedfrom,
                'link'        => '#taskid=' . $taskid,
            ];

            $notification_data['additional_data'] = serialize([
                get_task_subject_by_id($taskid),
            ]);
            ...
            echo json_encode(['success' => true, 'message' => _l('task_send_to_approval')]);
        }
    }
