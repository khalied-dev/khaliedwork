...
class Contracts extends AdminController
{
    ...
    /**   
     * Start get clint default currency.   
     *   
     * @author  @Ahmed Hmed   
     * @version 1.0   
     * @since   2023-8-2   
    ...
    public function get_client_default_currency()
    {
        if ($this->input->post()) {
            $client_id =  $this->input->post('client_id');
            $client = $this->clients_model->get($client_id);
            dd($client);
            if (isset($client) && $client->default_currency !== 0) {
                $default_currency = $this->db->get_where('tblcurrencies', ['id' => $client->default_currency])->row()->name ?? 'AED';
                echo json_decode($default_currency);
            } else {
                $default_currency = get_option('default_currency') == "" ? $default_currency = 'AED' : $default_currency = get_option('default_currency');
                echo json_decode($default_currency);
            }
        }
    }
