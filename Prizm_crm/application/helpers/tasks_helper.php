...
function get_sql_select_task_assignees_ids()
{
     ...
     *   
     * @author  @Osman Yahaya @Prizm Energy    
     * @version 1.0    
     * @since   2023-8-10  
     */
function get_sql_select_task_followers_full_names()
{
    return '(SELECT GROUP_CONCAT(CONCAT(firstname, \' \', lastname) ORDER BY ' . db_prefix() . 'task_followers.id ASC SEPARATOR ",") FROM ' . db_prefix() . 'task_followers JOIN ' . db_prefix() . 'staff ON ' . db_prefix() . 'staff.staffid = ' . db_prefix() . 'task_followers.staffid WHERE taskid=' . db_prefix() . 'tasks.id)';
}
...
/**
     ...
     * @since   2023-8-10    
     */ 
function get_sql_select_task_followers_ids()
{
    return '(SELECT GROUP_CONCAT(staffid ORDER BY ' . db_prefix() . 'task_followers.id ASC SEPARATOR ",") FROM ' . db_prefix() . 'task_followers WHERE taskid=' . db_prefix() . 'tasks.id)';
}
/** 
