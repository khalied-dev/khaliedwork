...
function get_proposal_count_by_status($statu)
{
    return $statu;
    $status = [
        'Draft' => 6,
    ...
    $CI = &get_instance();
    $CI->db->where('status', $status[$statu]);
    return $CI->db->count_all_results('tblproposals');
}

// get status approvel
function get_status_approval($status_id, $purchase_request_id = 4)
{
    $CI = &get_instance();
    $reques = $CI->db->get_where('tblprz_purchase_request', ['id', 1])->row();


    $data =  $CI->db->get_where('tblprz_purchase_request_statusdetail', [
        'statusID' => $status_id,
        'purchase_request_id' => $purchase_request_id,
        'stageID' => $reques->current_stage,
    ])->result();
...
function get_approver_information($staff_id)
{
    $CI = &get_instance();
    return $CI->db->get_where('tblstaff', ['staffid' => $staff_id])->row();
}
