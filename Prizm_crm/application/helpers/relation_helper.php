...
function get_relation_values($relation, $type)
{
    ...
    if ($type == 'customer' || $type == 'customers') {
        ...
        $name = format_opportunity_number($id);
        ...
        $link = admin_url('opportunities/list_active_oppurtunities/' . $id);
    } */ elseif ($type == 'tasks' || $type == 'task') {
        if (is_array($relation)) {
            $id   = $relation['id'];
            $name = $relation['name'];
...
function init_relation_options($data, $type, $rel_id = '')
{
    ...
    $has_permission_proposals_view = has_permission('opportunity', '', 'view');
    $is_admin                      = is_admin();
    $CI                            = & get_instance();
    $CI->load->model('projects_model');
