...
/**    
     ...
     * @version 1.0    
     * @since   2023-8-26
     * This file was added to enable preview of contract document in iframe    
     */ 
?>
...
<?php
$path = CONTRACTS_UPLOADS_FOLDER .  $file->rel_id . '/' . $file->file_name;


if (file_exists($path)) { if(is_image($path)){?> 
                  <img src="<?php echo base_url('uploads/contracts/' . $file->rel_id . '/' . $file->file_name); ?>" class="img img-responsive marginauto">
                  ...
                  <iframe src="<?php
                   echo base_url('uploads/contracts/' . $file->rel_id . '/' . $file->file_name); ?>" height="100%" width="100%" frameborder="0"></iframe>
                  <?php } else if (strpos($file->file_name, '.xls') !== false && empty($file->external)) {?>
                  <iframe src='https://view.officeapps.live.com/op/embed.aspx?src=<?php echo base_url('uploads/contracts/' . $file->rel_id . '/' . $file->file_name); ?>' width='100%' height='100%' frameborder='0'>
                  </iframe>
