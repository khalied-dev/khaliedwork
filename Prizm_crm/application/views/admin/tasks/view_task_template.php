    ...
    <div class="row">
        <div class="col-md-8 task-single-col-left">
            ...
            <?php if ($task->billed == 0) {
                ...
                <?php } ?>
                <p class="no-margin pull-left">
                    <a href="#" class="btn btn-default mleft5" data-toggle="tooltip" data-title="<?php echo _l('view_task_activity_log'); ?>" onclick="slideToggle('#view_task_activity_log'); return false;">
                        View Log
                    </a>
                </p>
            <?php
                                ...
                                <tr class="odd">
                                    <td colspan="5" class="add-timesheet">
                                        ...
                                        <div class="timesheet-duration hide">
                                            <div class="col-md-12">
                                                <i class="fa-regular fa-circle-question pointer pull-left mtop2" data-toggle="popover" data-html="true" data-content="
                                    ...
                                    "></i>
                                        ...
                                        <div class="col-md-12 text-right">
                                            ...
                                            <button <?php echo $disable_button; ?>data-task-id="<?php echo $task->id; ?>" class="btn btn-success task-single-add-timesheet"><i class="fa fa-plus"></i>
                                                <?php echo _l('submit'); ?></button>
                                        </div>
                                    </td>
                                </tr>
            ...
            <div class="clearfix"></div>
            <div id="view_task_activity_log" class="hide">

                <div class="widget" <?php if (count($task->timesheets) == 0) {
                                        echo ' hide';
                                    } ?>" id="widget-<?php echo create_widget_id(); ?>" data-name="<?php echo _l('task_activity_log'); ?>">
                        ...
                        <div class="panel-body padding-10">
                            ...
                            <div class="activity-feed">
                                ...
                                foreach ($tasklogs as $tasklog) {
                                    ...
                                    $name = $tasklog['firstname'] . ' ' . $tasklog['lastname'];


                                    if ($tasklog['staffid'] != 0) {
                                        $href = admin_url('profile/' . $tasklog['staffid']);
                                    } ?>
                                    <div class="feed-item">
                                        <div class="date"><span class="text-has-action" data-toggle="tooltip" data-title="<?php echo _dt($tasklog['adddate']); ?>">
                                                <?php echo time_ago(_dt($tasklog['adddate'])); ?>
                                            </span>
                                        </div>
                                        <div class="text">
                            ...
                            <?php else: ?>
                                <p style="color:red">No Task Activity Found...!!!!</p>
                            <?php endif; ?>
            ...
            <?php if (has_permission('tasks', '', 'edit')) { ?><a href="#" onclick="edit_task_inline_description(this,<?php echo $task->id; ?>); return false;" class="pull-left tw-mt-2.5 mleft5 font-medium-xs"><i class="fa-regular fa-pen-to-square"></i></a>
            <?php } ?>
            <div class="clearfix"></div>
        ...
        <div class="col-md-4 task-single-col-right">
            ...
            <p class="tw-mb-0 task-info-created tw-text-sm">
                <?php if (($task->addedfrom != 0 && $task->addedfrom != get_staff_user_id()) || $task->is_added_from_contact == 1) { ?>
                    <span class="tw-text-neutral-500"><?php echo _l('task_created_by', '<span class="tw-text-neutral-600">' . ($task->is_added_from_contact == 0 ? get_staff_full_name($task->addedfrom) : get_contact_full_name($task->addedfrom)) . '</span>'); ?>
                        <i class="fa-regular fa-clock" data-toggle="tooltip" data-title="<?php echo _l('task_created_at', _dt($task->dateadded)); ?>"></i></span>
                    <br />
                <?php } else { ?>
                    <span class="tw-text-neutral-500"><?php echo _l('task_created_at', '<span class="tw-text-neutral-600">' . _dt($task->dateadded) . '</span>'); ?></span>
                <?php } ?>
            </p>
            ...
            <?php if ($task->status == Tasks_model::STATUS_COMPLETE) { ?>
                <div class="task-info task-info-finished">
                    <h5 class="tw-inline-flex tw-items-center tw-space-x-1.5">
                        ...
                        <?php echo _l('task_single_finished'); ?>: <span data-toggle="tooltip" data-title="<?php echo _dt($task->datefinished); ?>" data-placement="bottom" class="text-has-action tw-text-neutral-800"><?php echo time_ago($task->datefinished); ?></span>
                    </h5>
                </div>
            <?php } ?>
            <div class="task-info task-single-inline-wrap task-info-start-date">
                <h5 class="tw-inline-flex tw-items-center tw-space-x-1.5">
                    ...
                    <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                        <input name="startdate" tabindex="-1" value="<?php echo _d($task->startdate); ?>" id="task-single-startdate" class="task-info-inline-input-edit datepicker pointer task-single-inline-field tw-text-neutral-800">
                    <?php } else { ?>
            ...
            <div class="task-info task-info-due-date task-single-inline-wrap<?php if (!$task->duedate && !has_permission('edit', '', 'tasks')) {
                ...
                <h5 class="tw-inline-flex tw-items-center tw-space-x-1.5">
                    ...
                    <?php if (has_permission('tasks', '', 'edit') && $task->status != 5) { ?>
                        <input name="duedate" tabindex="-1" value="<?php echo _d($task->duedate); ?>" id="task-single-duedate" class="task-info-inline-input-edit datepicker pointer task-single-inline-field tw-text-neutral-800" autocomplete="off" <?php if ($project_deadline) {
            ...
            <?php if (has_permission('tasks', '', 'create')) { ?>
                <div class="task-info task-info-total-logged-time">
                    <h5 class="tw-inline-flex tw-items-center tw-space-x-1.5">
                        <i class="fa-regular fa-clock fa-fw fa-lg task-info-icon"></i><?php echo _l('task_total_logged_time'); ?>
            ...
            foreach ($custom_fields as $field) { ?>
                ...
                <div class="task-info">
                    <h5 class="task-info-custom-field tw-inline-flex tw-items-center tw-space-x-1.5 task-info-custom-field-<?php echo $field['id']; ?>">
            ...
            <h4 class="task-info-heading tw-font-medium tw-text-base tw-flex tw-items-center tw-text-neutral-800">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="tw-w-5 tw-h-5 tw-text-neutral-500 tw-mr-2">
...
<script>
 ...
 /**   
     * Start task <Paste image in task> 3.0.6.   
     *   
     * @author  @Abdulrahman @Prizm   
     * @version 1.0   
     * @since   2023-OCT-12   
     */
    window.addEventListener("paste", e=>{
if(e.clipboardData.files.length>0){
    const fileInput=document.querySelector("#task-comment-form");
    fileInput.files=e.clipboardData.files;
    
    if(e.clipboardData.files[0].type.startsWith("image/")){
        let myDropzone =  Dropzone.forElement("#task-comment-form");
        myDropzone.on("paste", file => {
         console.log("A file has been pasted");
         });
        myDropzone.addFile(e.clipboardData.files[0]);
   }}
})
/**
     * End task <Past image in task> 3.0.6.
     */
