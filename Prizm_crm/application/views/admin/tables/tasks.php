...
$aColumns = [
    ...
    'duedate',
     ...
     /**    
     * Added the follower column updte 3.0.9.    
     *    
     * @author  @Osman Yahaya    
     * @version 1.0    
     * @since   2023-8-10    
     */
     ...
     get_sql_select_task_followers_full_names(). ' as followers',

      /** 
     * End follower column 3.0.9. 
     */
...
* @since   2023-SEP-8
*/ 
if ($this->ci->input->post('my_created_tasks')==1) {
    array_push($where, 'AND (' . db_prefix() . 'tasks.addedfrom = ' . get_staff_user_id() . ') and (' . db_prefix() . 'tasks.status not in (200,5) )');
}

if ($this->ci->input->post('my_follower_tasks')==1) {
    array_push($where, 'AND (' . db_prefix() . 'tasks.id IN (SELECT taskid FROM ' . db_prefix() . 'task_followers WHERE ' . db_prefix() . 'task_followers.staffid = ' . get_staff_user_id() . ')) and (' . db_prefix() . 'tasks.status not in (200,5) )');
}
...
/**   
* Start <Tasks Table Columns Search>.   
*   
* @author  @EhabIdris  @prizm 
* @version 1.0   
* @since   2023-10-09  
*/  
...
if ($this->ci->input->post('name')){
    ...
    array_push($where, "AND  " .db_prefix(). "tasks.name LIKE '%$name%'");

}

if ($this->ci->input->post('startdate')){
    $startdate = $this->ci->input->post('startdate');
    array_push($where, "AND  " .db_prefix(). "tasks.startdate LIKE '%$startdate%'");

}

if ($this->ci->input->post('duedate')){
    $duedate = $this->ci->input->post('duedate');
    array_push($where, "AND  " .db_prefix(). "tasks.duedate LIKE '%$duedate%'");

}

if ($this->ci->input->post('asignee')){
    $asignee = $this->ci->input->post('asignee');
    array_push($where, "AND " .db_prefix(). "tasks.id in (SELECT taskid from ".db_prefix()."task_assigned Join ".db_prefix()."staff on ".db_prefix()."task_assigned.staffid = ".db_prefix()."staff.staffid where firstname like '%$asignee%' or lastname like '%$asignee%')");
}


/**   
* End <Tasks Table Columns Search>.  
*/
...
$result = data_tables_init(
     ...
     */
        ...
        get_sql_select_task_followers_ids() . ' as followers_ids',
