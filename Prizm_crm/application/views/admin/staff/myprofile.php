...
<div id="wrapper">
    <div class="content">
        ...
        <div class="row">
            ...
            <div class="col-md-6<?php echo $this->input->get('notifications') ? ' hide' : ''; ?>">
                        ...
                        <div class="tw-flex">
                            ...
                            <div class="tw-space-x-0.5">
                                <?php if (!empty($staff_p->facebook)) { ?>
                                    <a href="<?php echo html_escape($staff_p->facebook); ?>" target="_blank" class="btn btn-default btn-icon">
                                        <i class="fa-brands fa-facebook-f"></i>
                                    </a>
                                <?php } ?>
                                <?php if (!empty($staff_p->linkedin)) { ?>
                                    <a href="<?php echo html_escape($staff_p->linkedin); ?>" class="btn btn-default btn-icon">
                                        <i class="fa-brands fa-linkedin-in"></i>
                                    </a>
                                <?php } ?>
                                <?php if (!empty($staff_p->skype)) { ?>
                                    <a href="skype:<?php echo html_escape($staff_p->skype); ?>" data-toggle="tooltip" title="<?php echo html_escape($staff_p->skype); ?>" target="_blank" class="btn btn-default btn-icon">
                                        <i class="fa-brands fa-skype"></i>
                                    </a>
                                <?php } ?>
                                <?php if (has_permission('staff', '', 'edit') && has_permission('staff', '', 'view')) { ?>
                                    <a href="<?php echo admin_url('staff/member/' . $staff_p->staffid); ?>" class="btn btn-default btn-icon">
            ...
            <?php if ($staff_p->staffid == get_staff_user_id()) { ?>
                <div class="col-md-6<?php if ($this->input->get('notifications')) {
                                        echo ' col-md-offset-2';
                                    } ?>">

                    <h4 class="tw-mt-0 tw-font-semibold tw-text-lg tw-text-neutral-700">
                        <?php echo _l('staff_profile_notifications'); ?>
                        <a href="#" class="tw-font-normal tw-text-sm" onclick="mark_all_notifications_as_read_inline(); return false;"><?php echo _l('mark_all_as_read'); ?></a>
                    </h4>
                    <div id="notifications" class="tw-my-3 tw-rounded-md tw-shadow-sm tw-overflow-hidden tw-divide-y tw-divide-solid tw-divide-neutral-200 tw-border tw-border-solid tw-border-neutral-200 [&_img]:tw-mr-3">
                    </div>
        ...
        <div class="row">
            ...
            <div class="col-md-6">
            <h4 class="tw-mt-0 tw-font-semibold tw-text-lg tw-text-neutral-700 tw-inline-flex tw-items-center">Your Task</h4>
                ...
                <div class="row">
                    <div class="col-md-2" hidden>
                        <?php if (!$this->input->get('project_id')) { ?>
                            <a href="<?php echo admin_url('tasks'); ?>" class="btn btn-default tw-mt-0.5"><?php echo _l('back_to_tasks_list'); ?></a>
                        <?php } else { ?>
                            <a href="<?php echo admin_url('projects/view/' . $this->input->get('project_id') . '?group=project_tasks'); ?>" class="btn btn-default tw-mt-0.5"><?php echo _l('back_to_project'); ?></a>
                        <?php } ?>
                    </div>
                    <?php echo form_hidden('project_id', $this->input->get('project_id')); ?>
                    <?php if (has_permission('tasks', '', 'view')) { ?>
                        <div class="col-md-2 border-right" hidden>
                            <?php
                    ...
                    <div class="col-md-3 border-right select-placeholder">
                ...
                <div class="panel_s tw-mt-8">
                    <div class="panel-body panel-table-full">
                        <?php foreach ($overview as $month => $data) {
                            if (count($data) == 0) {
                                echo '<table class="table tasks-overview dt-table">
                                <thead>
                                    <tr ><p style="color:red">No Task Found</p></tr> </thead></table>';
                                continue;
                            } ?>
                            ...
                            <table class="table tasks-overview dt-table">
                                <thead>
                                    <tr>
                                        ...
                                        <th><?php echo _l('Start/Due Dates'); ?></th>
                                    ...
                                    foreach ($data as $task) { ?>
                                        <tr>
                                            ...
                                            <td data-order="<?php echo $task['startdate']; ?>">
                                                <?php echo "Start Date: <br>";
                                                echo _d($task['startdate']);
                                                echo "<br>Due Date: <br>";
                                                echo _d($task['duedate']); ?>
