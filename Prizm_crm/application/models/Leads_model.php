...
/*         
  $this->db->select('*,' . db_prefix() . 'leads.name, ' 
        .db_prefix().'leads.id,'
        .db_prefix().'advance_leads.lead_id,'
        .db_prefix().'advance_leads_details.lead_id');


        $this->db->join(db_prefix().'advance_leads', db_prefix().'leads.id = '.db_prefix().'advance_leads.lead_id', 'left');
        $this->db->join(db_prefix().'advance_leads_details', db_prefix().'leads.id = '.db_prefix().'advance_leads_details.lead_id', 'left');

        $this->db->where($where); 
        
        print_r($result['advance_leads_details_res']);
        exit(0);
...
$result['leads_res'] = [];
$this->db->where($where);
$result['leads_res'] = $this->db->get(db_prefix() . 'leads')->result_array();

$result['advance_leads_res'] = [];
foreach($result['leads_res'] as $leads_res){
$this->db->where(["lead_id" => $leads_res['id']]);
$result['advance_leads_res'] = $this->db->get(db_prefix() . 'advance_leads')->result_array();
}

$result['advance_leads_details_res'] = [];
foreach($result['leads_res'] as $leads_res){
$this->db->where(["lead_id" => $leads_res['id']]);
$result['advance_leads_details_res'] = $this->db->get(db_prefix() . 'advance_leads_details')->result_array();
}
