...
class Dashboard_model extends App_Model
{
    ...
    public function departments_request_chart()
    {
        $departments_total_requests  =  $this->db->query('SELECT prq.*, dp.departmentid, dp.name as department,  SUM(prq.total) as total FROM ' . db_prefix() . 'pur_request prq LEFT JOIN ' . db_prefix() . 'departments dp ON prq.department = dp.departmentid GROUP BY dp.departmentid')->result_array();
        // dd($data);
        // var pieData = {
        //     labels: ['Label 1', 'Label 2', 'Label 3'],
        //     datasets: [{
        //         data: [30, 50, 20], // Replace with your data values
        ...
        $data = [];
        $backgroundColor = [];
        $colors = ['red', 'green', 'blue', 'yellow', 'orange', 'purple', 'pink', 'brown', 'grey', 'black', 'white'];
        $i = 0;
        foreach ($departments_total_requests as $requests) {

            array_push($lable, $requests['department']);
            array_push($data, $requests['total']);
            array_push($backgroundColor, $colors[$i]);
            $i++;
        }
        $datasets['data'] = $data;
        $datasets['backgroundColor'] = $backgroundColor;
        $datasets['hoverBackgroundColor'] = $backgroundColor;
        $datasets['label'] = 'Total Requests';
        $chart = [
            'labels'   => $lable,
            'datasets' => [$datasets],
        ];
