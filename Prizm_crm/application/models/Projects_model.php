...
class Projects_model extends App_Model
{
    ...
    public function add($data)
    {
        ...
        // start add project manager and senior manager v3.0
        $data['projectseniormanager']    = $data['projectseniormanager'];
        $data['projectmanager']    = $data['projectmanager'];
        // end add project manager and senior manager v3.0
    ...
    // start add project senior manager and project manager v3.3.0
    public function getSeniorProjectManager($id = '')
    {

        $this->db->select(db_prefix() . 'staff.employee_code,' . db_prefix() . 'staff.firstname,' . db_prefix() . 'staff.lastname,' . db_prefix() . 'staff.staffid');
        $this->db->from(db_prefix() . 'staff');
        $this->db->join(db_prefix() . 'projects', db_prefix() . 'projects.projectseniormanager = ' . db_prefix() . 'staff.staffid', 'inner');
    ...
    public function getProjectManager($id = '')
    {
        ...
        $this->db->select(db_prefix() . 'staff.employee_code,' . db_prefix() . 'staff.firstname,' . db_prefix() . 'staff.lastname,' . db_prefix() . 'staff.staffid');
        $this->db->from(db_prefix() . 'staff');
        $this->db->join(db_prefix() . 'projects', db_prefix() . 'projects.projectmanager = ' . db_prefix() . 'staff.staffid', 'inner');
        $this->db->where(db_prefix() . 'projects.id', $id);
        $query = $this->db->get();
        return $query->result();
    }

    // start add project senior manager and project manager v3.0.6
