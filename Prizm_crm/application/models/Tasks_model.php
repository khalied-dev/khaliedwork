...
class Tasks_model extends App_Model
{
    ...
    public function getTaskID($searchByidValue, $tableName, $columnName, $retrunColum)
    {

        $this->db->select(db_prefix() . $tableName . ".*");
        $this->db->from(db_prefix() . $tableName);
        $this->db->where(db_prefix() . $tableName . "." . $columnName, $searchByidValue);

        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            $row = $query->row_array();
            if (isset($row[$retrunColum]))
                return $row[$retrunColum];
        }
